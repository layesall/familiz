...
func MemberPDFHandler(w http.ResponseWriter, r *http.Request) {
	...
	txs, err := contributions.GetContributionsByMemberID(id, includeArchived)
	if err != nil {
		http.Error(w, "Erreur récupération contributions", http.StatusInternalServerError)
		return
	}
