...
func CreateEventHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Type != "wedding" && req.Type != "baptism" {
		http.Error(w, "type invalide (wedding/baptism)", http.StatusBadRequest)
		return
	}
	if req.EventDate == "" {
		http.Error(w, "event_date est obligatoire", http.StatusBadRequest)
		return
	}

	// Calcul auto du montant (service)
	finalAmount, err := CalculateEventAmount(req.Type, req.AmountReceived)
	if err != nil {
